...
class PengirimanBarangController extends Controller
{
    ...
    public function getHargaBarang(Request $request)
    {
        ...
        try {
            ...
            if ($id_toko == 1) {
                // Cek stok dari DetailStockBarang
                $stock = DetailStockBarang::where('id_barang', $id_barang)
                    ->where(function ($q) use ($id_supplier) {
                ...
                if ($stock && $stock->qty_now > 0) {
                    ...
                    return response()->json([
                        ...
                        'data' => [
                            ...
                            'id_detail' => $id_detail ?: $stock->id_detail_pembelian,
                            'nama_supplier' => optional($stock->supplier)->nama_supplier ?? '-', // fallback
                ...
                // Jika bukan toko utama, cek stok dari detail_toko
                ...
                if ($stockToko && $stockToko->qty > 0) {
                    return response()->json([
                        ...
                        'data' => [
                            ...
                            'nama_supplier' => optional($barang->supplier)->nama_supplier ?? '-', // fallback
    ...
    public function edit($id)
    {
        ...
        $pengiriman_barang = PengirimanBarang::with(['detail.supplier'])->findOrFail($id);
        $userTokoId = Auth::user()->id_toko;
        ...
        // Map supplier name jika null
        $pengiriman_barang->detail->transform(function ($item) {
            $item->nama_supplier = $item->supplier->nama_supplier ?? 'Tidak ada';
            return $item;
        });
    ...
    public function storetempPengiriman(Request $request)
    {
        $request->validate([
            ...
            // id_supplier tidak wajib jika bisa null
        ]);
        ...
        try {
            DB::table('temp_detail_pengiriman')->insert([
                ...
                'id_barang' => $request->id_barang,
                'id_supplier' => $request->id_supplier ?: null, // fallback null jika kosong
                'qty' => $request->qty,
                'harga' => $request->harga,
    ...
    public function updatetempPengiriman(Request $request)
    {
        $request->validate([
            ...
            'harga' => 'required|numeric|min:0',
            'id_pengiriman_barang' => 'required',
            // 'id_supplier' tidak required karena bisa null
        ]);

        $totalharga = $request->qty * $request->harga;

        try {
            $query = DB::table('temp_detail_pengiriman')
                ->where('id_pengiriman_barang', $request->id_pengiriman_barang)
                ->where('id_barang', $request->id_barang);

            // Tangani kondisi id_supplier bisa null
            if ($request->filled('id_supplier')) {
                $query->where('id_supplier', $request->id_supplier);
            } else {
                $query->whereNull('id_supplier');
            }
        ...
        try {
            ...
            if ($request->status == 'success' || $request->status == 'progress') {
                $data = DetailPengirimanBarang::where('id_pengiriman_barang', $request->id_pengiriman_barang)
                    ...
                    ->leftJoin('supplier', 'detail_pengiriman_barang.id_supplier', '=', 'supplier.id')
                ...
                $data = DB::table('temp_detail_pengiriman')
                    ...
                    ->leftJoin('supplier', 'temp_detail_pengiriman.id_supplier', '=', 'supplier.id')
                    ...
                    ->select(
                        'temp_detail_pengiriman.*',
                        'barang.nama_barang',
                        'supplier.nama_supplier',
                        'detail_stock.qty_now as stock',
                        'detail_pembelian_barang.qrcode'
                    )
        ...
        try {
            ...
            foreach ($idBarangs as $index => $id_barang) {
                ...
                DetailPengirimanBarang::create([
                    ...
                    'id_supplier' => ($request->id_supplier[$index] !== 'null' && $request->id_supplier[$index] !== '')
                        ? $request->id_supplier[$index]
                        : null,
                ]);
